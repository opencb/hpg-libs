...
sw_mem_i16_t *sw_mem_i16_new() {
  ...
  mem->q_size = len;
  mem->qq = (short *) _mm_malloc(SIMD_DEPTH * len * sizeof(short), SIMD_ALIGNED);

  mem->r_size = len;
  mem->rr = (short *) _mm_malloc(SIMD_DEPTH * len * sizeof(short), SIMD_ALIGNED);

  mem->H_size = len * len;
  mem->H = (short *) _mm_malloc(SIMD_DEPTH * mem->H_size * sizeof(short), SIMD_ALIGNED);
  mem->C = (short *) _mm_malloc(SIMD_DEPTH * mem->H_size * sizeof(short), SIMD_ALIGNED);

  mem->F_size = len;
  mem->F = (short *) _mm_malloc(SIMD_DEPTH * len * sizeof(short), SIMD_ALIGNED);
...
void sw_mem_i16_alloc(char **q, char **r, int num_seqs,
		      sw_mem_i16_t *mem) {
  int len, i, j;
  int max_q_len = 0;
  int max_r_len = 0;
  ...
  if (max_q_len > mem->q_size) {
    _mm_free(mem->qq);
    mem->qq = (short *) _mm_malloc(SIMD_DEPTH * max_q_len * sizeof(short), SIMD_ALIGNED);
  ...
  if (max_r_len > mem->r_size) {
    _mm_free(mem->rr);
    mem->rr = (short *) _mm_malloc(SIMD_DEPTH * max_r_len * sizeof(short), SIMD_ALIGNED);
  ...
  for(i=0; i < SIMD_DEPTH; i++) {
    ...
    for(j=0; j < mem->q_lens[i]; j++) {
      mem->qq[SIMD_DEPTH * j + i] = (short) q[i][j];
    }
    for(; j < max_q_len; j++) {
      mem->qq[SIMD_DEPTH * j + i] = ';';
    }
    for(j=0; j < mem->r_lens[i]; j++) {
      mem->rr[SIMD_DEPTH * j + i] = (short) r[i][j];
    }
    for(; j < max_r_len; j++) {
  ...
  if (matrix_size > mem->H_size) {
    ...
    mem->H = (short *) _mm_malloc(size * sizeof(short), SIMD_ALIGNED);
    mem->C = (short *) _mm_malloc(size * sizeof(short), SIMD_ALIGNED);
    //printf("new H %x, C %x\n", *H, *C);
  ...
  if (max_q_len > mem->F_size) {
    _mm_free(mem->F);
    mem->F = (short *) _mm_malloc(SIMD_DEPTH * max_q_len * sizeof(short), SIMD_ALIGNED);
