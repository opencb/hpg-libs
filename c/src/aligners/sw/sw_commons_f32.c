...
//-------------------------------------------------------------------------
// sw_mem_f32_t functions
//-------------------------------------------------------------------------

sw_mem_f32_t *sw_mem_f32_new() {
  const int len = 250;
  sw_mem_f32_t *mem = (sw_mem_f32_t *) malloc(sizeof(sw_mem_f32_t));
  mem->q_size = len;
  mem->qq = (int *) _mm_malloc(SIMD_DEPTH * len * sizeof(int), SIMD_ALIGNED);

  mem->r_size = len;
  mem->rr = (int *) _mm_malloc(SIMD_DEPTH * len * sizeof(int), SIMD_ALIGNED);

  mem->H_size = len * len;
  mem->H = (float *) _mm_malloc(SIMD_DEPTH * mem->H_size * sizeof(float), SIMD_ALIGNED);
  mem->C = (int *) _mm_malloc(SIMD_DEPTH * mem->H_size * sizeof(int), SIMD_ALIGNED);

  mem->F_size = len;
  mem->F = (float *) _mm_malloc(SIMD_DEPTH * len * sizeof(float), SIMD_ALIGNED);
...
void sw_mem_f32_alloc(char **q, char **r, int num_seqs,
		      sw_mem_f32_t *mem) {
  int len, i, j;
  int max_q_len = 0;
  int max_r_len = 0;
  ...
  if (max_q_len > mem->q_size) {
    _mm_free(mem->qq);
    mem->qq = (int *) _mm_malloc(SIMD_DEPTH * max_q_len * sizeof(int), SIMD_ALIGNED);
  ...
  if (max_r_len > mem->r_size) {
    _mm_free(mem->rr);
    mem->rr = (int *) _mm_malloc(SIMD_DEPTH * max_r_len * sizeof(int), SIMD_ALIGNED);
  ...
  for(i=0; i < SIMD_DEPTH; i++) {
    for(j=0; j < mem->q_lens[i]; j++) {
      mem->qq[SIMD_DEPTH * j + i] = (int) q[i][j];
    }
    for(; j < max_q_len; j++) {
      mem->qq[SIMD_DEPTH * j + i] = ';';
    }
    for(j=0; j < mem->r_lens[i]; j++) {
      mem->rr[SIMD_DEPTH * j + i] = (int) r[i][j];
    }
    for(; j < max_r_len; j++) {
  ...
  if (matrix_size > mem->H_size) {
    ...
    mem->H = (float *) _mm_malloc(size * sizeof(float), SIMD_ALIGNED);
    mem->C = (int *) _mm_malloc(size * sizeof(int), SIMD_ALIGNED);
    //printf("new H %x, C %x\n", *H, *C);
  ...
  if (max_q_len > mem->F_size) {
    _mm_free(mem->F);
    mem->F = (float *) _mm_malloc(SIMD_DEPTH * max_q_len * sizeof(float), SIMD_ALIGNED);
