...
//-------------------------------------------------------------------------
// sw_mem_i32_t functions
//-------------------------------------------------------------------------

sw_mem_i32_t *sw_mem_i32_new() {
  const int len = 250;
  sw_mem_i32_t *mem = (sw_mem_i32_t *) malloc(sizeof(sw_mem_i32_t));
  mem->q_size = len;
  ...
  mem->H_size = len * len;
  mem->H = (int *) _mm_malloc(SIMD_DEPTH * mem->H_size * sizeof(int), SIMD_ALIGNED);
  mem->C = (int *) _mm_malloc(SIMD_DEPTH * mem->H_size * sizeof(int), SIMD_ALIGNED);

  mem->F_size = len;
  mem->F = (int *) _mm_malloc(SIMD_DEPTH * len * sizeof(int), SIMD_ALIGNED);
...
void sw_mem_i32_alloc(char **q, char **r, int num_seqs,
		      sw_mem_i32_t *mem) {
  int len, i, j;
  int max_q_len = 0;
  int max_r_len = 0;
  ...
  if (matrix_size > mem->H_size) {
    ...
    mem->H = (int *) _mm_malloc(size * sizeof(int), SIMD_ALIGNED);
  ...
  if (max_q_len > mem->F_size) {
    _mm_free(mem->F);
    mem->F = (int *) _mm_malloc(SIMD_DEPTH * max_q_len * sizeof(int), SIMD_ALIGNED);
