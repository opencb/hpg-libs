...
void fastq_reads_stats_free(array_list_t *fq_reads_stats) {
	if(fq_reads_stats != NULL) {
		...
		//		for(int i=0; i<fq_reads_stats->size; i++) {
		//			if(array_list_get(i, fq_reads_stats) != NULL) {
		//				free(array_list_get(i, fq_reads_stats));
		//			}
		//		}
	}
}
...
int fastq_read_stats_se(fastq_read_t *fq_read, fastq_read_stats_options_t *fq_read_stats_options, fastq_read_stats_t *fq_read_stats) {
	if(fq_read != NULL) {
		fastq_read_stats_init(fq_read_stats);
		for(size_t i=0; i<fq_read->length; i++) {
			...
			if(fq_read->quality[i] < fq_read_stats_options->min_qual || fq_read->quality[i] > fq_read_stats_options->max_qual) {
...
void fastq_read_stats_print(fastq_read_stats_t *fq_read_stats) {
	printf("Quality avg.: %f, A,T,C,G: %i,%i,%i,%i, Ns: %i\n", fq_read_stats->quality_average, fq_read_stats->num_A, fq_read_stats->num_T, fq_read_stats->num_C, fq_read_stats->num_G, fq_read_stats->Ns);
}
