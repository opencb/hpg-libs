...
typedef struct sw_simd_input {
     ...
     unsigned int curr_depth; /**< Number of queries. */
...
typedef struct sw_simd_output {
  ...
  unsigned int* mapped_seq_len_p; /**< Pointers to the aligned target sequences lengths. */
  unsigned int* mapped_ref_len_p; /**< Pointers to the aligned reference sequences lenghts. */
...
//void smith_waterman_simd(char** query_p, char** ref_p, sw_simd_output_t* output_p, sw_simd_context_t* context_p);
