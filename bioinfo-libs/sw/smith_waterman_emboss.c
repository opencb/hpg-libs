...
void sw_simd_input_add(char* seq_p, unsigned int seq_len, char* ref_p,
		       unsigned int ref_len, sw_simd_input_t* input_p) {

    unsigned int curr_depth = ++input_p->curr_depth;
    if (curr_depth >= input_p->depth) {
        printf("ERROR: out of range!\n");
	return;
    }
     
    input_p->seq_p[curr_depth] = seq_p;
    input_p->ref_p[curr_depth] = ref_p;
    input_p->seq_len_p[curr_depth] = seq_len;
    input_p->ref_len_p[curr_depth] = ref_len;
}
...
sw_simd_output_t* sw_simd_output_new(unsigned int depth) {
     ...
     output_p->mapped_seq_len_p = (unsigned int*) calloc(depth, sizeof(unsigned int));
...
void sw_simd_output_free(sw_simd_output_t* output_p) {
     ...
     if (output_p->mapped_seq_len_p != NULL) free(output_p->mapped_seq_len_p);
...
void sw_simd_output_display(unsigned int depth, sw_simd_output_t* output_p) {
     ...
     for(int i = 0; i < depth; i++) {
	  ...
	  memcpy(str, output_p->mapped_seq_p[i], output_p->mapped_seq_len_p[i]);
	  str[output_p->mapped_seq_len_p[i]] = '\0';
	  printf("%i:\n", i);
	  printf("seq: %s (len = %i)\n", str, output_p->mapped_seq_len_p[i]);
	  
	  memcpy(str, output_p->mapped_ref_p[i], output_p->mapped_ref_len_p[i]);
	  str[output_p->mapped_ref_len_p[i]] = '\0';
	  printf("ref: %s (len = %i, start = %i)\n", str, output_p->mapped_ref_len_p[i], output_p->start_p[i]);
	  
	  printf("dif: ");
	  for(int j = 0; j < output_p->mapped_ref_len_p[i]; j++) {
...
sw_simd_context_t* sw_simd_context_new(subst_matrix_t *subst_matrix_p, float gap_open, float gap_extend) {
...
void sw_simd_context_free(sw_simd_context_t* context_p) {
  ...
  /*
  _mm_free(context_p->zero_p);
  _mm_free(context_p->gap_open_p);
  _mm_free(context_p->gap_extend_p);
  */
  if (context_p->E != NULL) _mm_free(context_p->E);
  if (context_p->F != NULL) _mm_free(context_p->F);
  if (context_p->H != NULL) _mm_free(context_p->H);
...
void smith_waterman_simd(char** query_p, char** ref_p, 
...
*/
     ...
     */
	  ...
	  *b_mapp = 0;
	  ...
	  output_p->mapped_seq_len_p[i] = a_map + max_size - a_mapp;
	  output_p->mapped_seq_p[i] = (char*) calloc(output_p->mapped_seq_len_p[i], sizeof(char));
	  memcpy(output_p->mapped_seq_p[i], a_mapp, output_p->mapped_seq_len_p[i]);
	  
	  //output_p->mapped_ref_p[i] = strdup(b_mapp);
	  output_p->mapped_ref_len_p[i] = b_map + max_size - b_mapp;
	  output_p->mapped_ref_p[i] = (char*) calloc(output_p->mapped_ref_len_p[i], sizeof(char));
	  memcpy(output_p->mapped_ref_p[i], b_mapp, output_p->mapped_ref_len_p[i]);
	  
	  output_p->norm_score_p[i] = output_p->score_p[i] / (output_p->mapped_seq_len_p[i] * context_p->substitution[1]);
     } // end of for 0..SIMD_DEPTH
}
