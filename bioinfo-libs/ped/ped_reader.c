...
#line 15 "ped_reader.c"
static const int ped_start = 1;
static const int ped_first_final = 29;
static const int ped_error = 0;

static const int ped_en_header_scan = 32;
static const int ped_en_record_scan = 34;
...
static void set_field(char* ts, char *te)
{
    ...
    switch (current_field)
    {
        ...
        break;
        case START:
            set_ped_record_sex(atol(field), current_record);
            free(field);    // Not set as ped_record_t variable -> not freed later
        break;
        case END:
            set_ped_record_phenotype(atol(field), current_record);
            free(field);    // Not set as ped_record_t variable -> not freed later
        break;
        case SCORE:
...
#line 35 "ped.ragel"
	{te = p+1;{
            add_ped_header_entry(current_header_entry, file);
            current_header_entry = create_ped_header_entry();
            LOG_DEBUG("\n");
...
#line 30 "ped.ragel"
	{te = p;p--;{
            ...
            set_ped_header_entry_text(text, current_header_entry);
        }}
	goto st32;
st32:
#line 1 "NONE"
