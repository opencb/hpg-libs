...
write_batch_t* write_batch_new(unsigned int size, unsigned char flag) {
  ...
  if(flag != MATCH_FLAG){
    write_batch_p->allocated_size = size;
    write_batch_p->buffer_p = (void *) calloc(size, sizeof(char));
  }else{
    write_batch_p->allocated_size = size/sizeof(alignment_t *);
