...
env = Environment(tools = ['default', 'packaging'],
      		  CC = compiler,
                  ...
                  CPPPATH = ['#', '#src', '#include', '$CONFIG_INCLUDE_PATH', '$SAMTOOLS_INCLUDE_PATH', '$CPROPS_INCLUDE_PATH', '$ARGTABLE_INCLUDE_PATH', bioinfo_path, commons_path ],
#                  CPPPATH = ['#', '#src', '#include', '/usr/local/include', '/usr/include/libxml2', '$CPROPS_INCLUDE_PATH', '$ARGTABLE_INCLUDE_PATH', bioinfo_path, commons_path, math_path ],
                  LIBPATH = ['#libs/common-libs/', '$CONFIG_LIBRARY_PATH', '$SAMTOOLS_LIBRARY_PATH', '$CPROPS_LIBRARY_PATH', '$ARGTABLE_LIBRARY_PATH', commons_path ],
